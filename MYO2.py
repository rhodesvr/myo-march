...
import GLOBALS
sys.path.append(os.path.join(GLOBALS.MYO_LOCATION))
sys.path.append(os.path.join(GLOBALS.MYO_SDK_LOCATION))
import myo
...
if ori.getSensor().getDisplayMode() == oculus.DISPLAY_DESKTOP:
...
if recordingEnabled:
    ...
    if(not os.path.exists(dirPath)):
        os.makedirs(dirPath)
    recordFile = open('.\Results\Subject_%s\myo_%s.txt' % (str(subjectID), subjectInitial), 'w')
...
masterObjectLocations = [[ -1.04082/scale, (obj_h-0.9)* scale_h, 24/scale ],\
 [ -8.44218/scale, (obj_h-1)*scale_h, 4.5714/scale ],\
 [ -15.2653/scale, (obj_h-1.3)*scale_h , -9.90476/scale ],\
 [ 1.04082/scale, (obj_h-0.6)*scale_h, -11.8095/scale ],\
 [ 12.6054/scale, (obj_h-1)*scale_h , -21.7143/scale ],\
 [ (15.6122 + xdif)/scale, (obj_h-1.15)*scale_h, (-9.71429 + xdif * slope35)/scale ]]
...
masterTargetLocations = [[ -17/scale, test_h, 1.333333/scale ],\
[ -7.17007/scale, test_h, -11.2381/scale ],\
[ 1.04082/scale, test_h, -20.8095/scale ],\
[ -18/scale, test_h, 9/scale ],\
[ 6.82313/scale, test_h, -3.80952/scale ],\
[ 15.2653/scale, test_h, 7.47619/scale ]]
...
masterArrowLocations = [[(masterTargetLocations[0][0])/scale, arrow_h, (masterTargetLocations[0][2]+offset)/scale],\
[(masterTargetLocations[1][0] + math.cos(math.pi/4.25) * offset)/scale, arrow_h, (masterTargetLocations[1][2] + math.sin(math.pi/4.25) * offset)/scale],\
[(masterTargetLocations[2][0] + offset)/scale, arrow_h, (masterTargetLocations[2][2])/scale],\
[(masterTargetLocations[3][0])/scale, arrow_h, (masterTargetLocations[3][2] - offset)/scale],\
[(masterTargetLocations[4][0] - offset)/scale, arrow_h, (masterTargetLocations[4][2] - slope4 * offset)/scale],\
[(masterTargetLocations[5][0] - offset)/scale, arrow_h, (masterTargetLocations[5][2])/scale]]
...
def statusUpdate():
    ...
    global currentTarget, set, recordFile
