...
view = viz.MainView
hmd = oculus.Rift() #initialize oculus
vrpn = viz.add('vrpn7.dle') #open vrpn to get position data
pos = vrpn.addTracker('PPT0@WORLDVIZ-PC', 0) #position gathered from ppt cameras
ori = hmd.getSensor() #orientation gathered from oculus
comp = viz.mergeLinkable(pos, ori) #composite tracker of position and orientation
head = viz.link(comp, view) #link composite tracker to main view
hmd.window = view
...
#viz.phys.enable()
#viz.phys.setGravity(0,-9.8,0) #give objects gravity, probs won't need much object physics after hard code of height
...
if recordingEnabled:
	...
	if(not os.path.exists(dirPath)):
		os.makedirs(dirPath)
	recordFile = open('Results\\Subject_%s\\PPT_%s.txt' % (str(subjectID), subjectInitial), 'w')
...
def initializeLocations():
	...
	for x in range(len(masterObjectLocations)):
		...
		objects[x].setScale([0.45,0.45,0.45])
		#objects[x].collideBox()
